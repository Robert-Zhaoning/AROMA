...
PAIR_PATH = Path(
    "outputs/proc_count_causal_v1/exact_matched_pairs_eager.csv"
)
...
def load_heads():
    ...
    # Formal adaptive-router probe uses only the seven
    # candidate heads frozen before held-out evaluation.
    #
    # Random controls remain part of the earlier causal
    # validation experiments but are NOT router features.
...
def main(limit=None, protocol_only=False):
    ...
    if len(pairs) != 27:
        raise RuntimeError(
            "Expected exactly 27 held-out pairs after "
            f"excluding discovery pair {DISCOVERY_PAIR_ID}, "
    ...
    if len(heads) != 7:
        raise RuntimeError(
            f"Expected 7 frozen candidate heads, found {len(heads)}."
        )

    if any(
        head_type != "candidate"
        for head_type, _, _ in heads
    ):
        raise RuntimeError(
            "Non-candidate head survived formal protocol."
        )
    ...
    if len(sample_ids) != 54:
        raise RuntimeError(
            f"Expected 54 held-out samples, found {len(sample_ids)}."
        )

    if not HELDOUT_CLEAN_PATH.exists():
        raise FileNotFoundError(
            HELDOUT_CLEAN_PATH
        )

    heldout_clean = pd.read_csv(
        HELDOUT_CLEAN_PATH
    )

    expected_sample_ids = set(
        heldout_clean["sample_id"]
        .astype(str)
        .tolist()
    )
    ...
    if len(expected_sample_ids) != 54:
        raise RuntimeError(
            "Clean held-out feature file does not contain "
            "exactly 54 unique samples."
        )
    ...
    if actual_sample_ids != expected_sample_ids:
        ...
        raise RuntimeError(
            "Formal probe sample set does not match the "
            "audited held-out sample set. "
    ...
    if leaked_discovery:
        raise RuntimeError(
            "Discovery sample leakage detected: "
            f"{sorted(leaked_discovery)}"
        )

    print(
        "Held-out sample-set audit: PASS"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Audit the formal held-out protocol and exit "
